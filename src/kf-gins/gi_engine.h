...
public:
    ...
    /**
     * @brief 获取最近一次真正用于GNSS更新的标准差
     * */
    Eigen::Vector3d getLastGnssStd() const {
        return last_gnss_std_;
    }

    /**
     * @brief 是否已有最近一次GNSS更新标准差
     * */
    bool hasLastGnssStd() const {
        return has_last_gnss_std_;
    }
...
private:
    ...
    // 最近一次真正用于GNSS更新的标准差
    Eigen::Vector3d last_gnss_std_ = Eigen::Vector3d::Zero();
    bool has_last_gnss_std_ = false;
